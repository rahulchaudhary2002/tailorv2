    ...
    <div class="table-card" style="margin-bottom: 16px;">
        ...
        <div class="flex flex-wrap gap-6" style="padding: 16px;">
            <div class="outlet-form-group">
                <label>Order Number: <span style="font-weight: bold;">{{ $order->order_number }}</span></label>
            </div>
            <div class="outlet-form-group">
                <label>Status: <span class="app-badge {{ $order->displayStatusBadgeClass() }}">
                        {{ $order->displayStatusLabel() }}
                    </span></label>
            </div>
            <div class="outlet-form-group">
                <label>Payment Status: <span class="app-badge {{ \App\Models\Order::paymentStatusBadgeClass((string) $order->payment_status) }}">
                        {{ \App\Models\Order::paymentStatusLabel((string) $order->payment_status) }}
                    </span></label>
            </div>
            <div class="outlet-form-group">
                <label>Outlet: <span style="font-weight: bold;">{{ $order->outlet?->name ?? '-' }}</span></label>
            </div>
            <div class="outlet-form-group">
                <label>Ordered At: <span style="font-weight: bold;">{{ $order->ordered_at?->format('M d, Y h:i A') ?? '-' }}</span></label>
            </div>
            <div class="outlet-form-group">
                <label style="display: flex; align-items: center; gap: 8px;">Delivery Due: <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
                        <span style="font-weight: bold;">{{ $order->delivery_due_at?->format('M d, Y h:i A') ?? '-' }}</span>
            ...
            <div class="outlet-form-group">
                <label>Task Workers: @if ($taskWorkers->isNotEmpty())
                    @foreach ($taskWorkers as $worker)
                    @canany(['view-task-management', 'manage-task-management', 'manage-orders'])
                    <a href="{{ route('worker.tasks', $worker) }}" style="text-decoration: underline; font-weight: bold;">
                        {{ $worker->name }}
                    </a>@if (! $loop->last), @endif
                    @else
                    <span style="font-weight: bold;">{{ $worker->name }}</span>@if (! $loop->last), @endif
                    @endcanany
                    @endforeach
                    @else
            ...
            <div class="outlet-form-group">
                <label>Task Deadline: <span style="font-weight: bold;">{{ $taskDeadline?->format('M d, Y h:i A') ?? '-' }}</span></label>
            </div>
            <div class="outlet-form-group">
                <label>Created By: <span style="font-weight: bold;">{{ $order->creator?->name ?? '-' }}</span></label>
            </div>
            <div class="outlet-form-group outlet-form-group-full">
                <label>Notes: <span style="font-weight: bold;">{{ $order->notes ?: '-' }}</span></label>
            </div>
        </div>
    </div>
